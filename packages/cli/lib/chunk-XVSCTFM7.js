import {
  buildInstallCommand as c,
  buildUninstallCommand as m,
} from '@stricli/auto-complete'
import { buildRouteMap as f, buildApplication as u } from '@stricli/core'
import { buildCommand as l } from '@stricli/core'
import { buildCommand as p } from '@stricli/core'
import { buildCommand as d } from '@stricli/core'
import g from 'node:fs'
import h from 'node:os'
import y from 'node:path'

var t = '@saas-ui/cli'
var r = 'Saas UI command line'
var i = '0.0.1'
import { buildCommand as l } from '@stricli/core'
var n = l({
  loader: async () => {
    let { add: e } = await import('./impl-LWIEP6W5.js')
    return e
  },
  parameters: {
    flags: {
...
var a = p({
  loader: async () => {
    let { init: e } = await import('./impl-VDXYRJRU.js')
    return e
  },
  parameters: {
    flags: {
...
var s = d({
  loader: async () => {
    let { login: e } = await import('./impl-7GCY5DW7.js')
    return e
  },
  parameters: {},
  docs: { brief: 'Log in with your Saas UI account.' },
})
var b = f({
    routes: {
      login: s,
      init: a,
      add: n,
      install: c('cli', { bash: '__cli_bash_complete' }),
      uninstall: m('cli', { bash: !0 }),
    },
    docs: { brief: r, hideRoute: { install: !0, uninstall: !0 } },
  }),
  L = u(b, { name: t, versionInfo: { currentVersion: i } })
import g from 'node:fs'
import h from 'node:os'
import y from 'node:path'
function O(e) {
  return { process: e, os: h, fs: g, path: y }
}
